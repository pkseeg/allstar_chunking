...
static int process_directory(const char* current, const char* path, SETTINGS* settings)
{
    ...
    if (chdir(path) == 0)
    {
        ...
        if (directory != NULL)
        {
            ...
            do
            {
                ...
                if (dir > 0)
                {
                    ...
                    {
                        result = process_directory(full_path, directory->name,
                            settings);
                    }
                }
                else if (!dir)
                {
                    if(match(settings->pattern, directory->name)
                        && (add_to_list(&file_list, directory->name) < 0))
                    {
                        result = -1;
                    }
                }
        ...
        if ((result == 0) && (file_list != NULL))
        {
            ...
            result = process_files(file_list, settings);
        }
